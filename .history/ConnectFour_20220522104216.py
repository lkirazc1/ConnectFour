            ...
            if event.key == pygame.K_SPACE or event.key == pygame.K_RETURN:
                ...
                if cr is not None:
                    ...
                    if check_done(board, row, column, board[row][column]) or 0 not in board:
                        ...
                        pygame.draw.circle(display, (255,255,255), (1378//2, 90), 75)
                        text = font.render(who_wins, True, color)
                        display.blit(text, (text.get_rect(center=(1378/2, 90))))
                        pygame.display.update()
                        while not done:
                            for event in pygame.event.get():
                                if event.type == pygame.QUIT:
                    ...
                    update(presses, color)
    update(presses, (255, 255, 0) if color_number == -1 else (255, 255, 0))
    pygame.display.update()
