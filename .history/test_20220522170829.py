...
class TestGridMethods(unittest.TestCase):
    ...
    def test_diagonally_right(self):
        ...
        for i in range(4):
            for j in range(i + 1):
                if i == j:
                    last_place = self.grid.place_piece(i, 1)
                else:
                    last_place = self.grid.place_piece(i, -1)
                if i != 3:
                    self.assertFalse(self.grid.check_done(last_place[0], last_place[1], self.grid.get_color(last_place[0], last_place[1])))
        self.assertTrue(self.grid.check_done(last_place[0], last_place[1], self.grid.get_color(last_place[0], last_place[1])))
        
        for i in range(self.grid.rows):
            print("[", end="")
            for j in range(self.grid.cols):
                print(self.grid.get_color(i, j), end="")
            print("]")
        print("this is diagonal right")
    
    def test_diagonally_left(self):
        self.grid.clear()
