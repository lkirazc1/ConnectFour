...
class Game:
    ...
    def train_ai(self, genome1, genome2, config):
        ...
        while not done:
            ...
            place1 = self.grid.place_piece(output1.index(max(output1)), YELLOW)
            while place1 is None:
                genome1.fitness -= 1
                output1.remove(output1.index(max(output1)))
                place1 = self.grid.place_piece(output1.index(max(output1)), YELLOW)
            ...
            place2 = self.grid.place_piece(output2.index(max(output2)), YELLOW)
            while place2 is None:
                genome2.fitness -= 1
                output2.remove(output2.index(max(output2)))
                place1 = self.grid.place_piece(output2.index(max(output1)), YELLOW)
