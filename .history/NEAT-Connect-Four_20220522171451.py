...
class Game:
    ...
    def unpack_board(self):
        return [self.grid.board.get_color(r, c) for r in range(self.rows) for c in range(self.cols)]
    ...
    def train_ai(self, genome1, genome2, config):
        ...
        while not done:
            ...
            while place1 is None:
                genome1.fitness -= 1
                output1.pop(output1.index(max(output1)))
                place1 = self.grid.place_piece(output1.index(max(output1)), YELLOW)
            ...
            while place2 is None:
                genome2.fitness -= 1
                output2.pop(output2.index(max(output2)))
                place2 = self.grid.place_piece(output2.index(max(output2)), RED)
