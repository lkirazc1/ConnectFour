...
def check_done(board: np.ndarray, row: int, column: int, color: int) -> int:
    return cConnectFour.check_done(board.tolist(), row, column, color)

def next_move(board: np.ndarray, color: int, last_col: int, last_row: int, time_start: float) -> tuple:
    can_continue_thinking = time.time() - time_start < THINK_S
    ...
    if color == RED:  # red = computer so maximize the score
        ...
        for column in COMPUTER_COLUMN_ORDER:
            if board[0, column] != 0:
                continue
            if not can_continue_thinking:
                return (0, column)
            row, place_col = place_piece(board, column, RED)
            (best_score, best_move) = next_move(board, YELLOW, column, row, time_start)
            board[row, column] = 0
        ...
        for column in COMPUTER_COLUMN_ORDER:
            ...
            (worst_score, worst_move) = next_move(board, RED, column, row, time_start)
            board[row, column] = 0
            ...
            if event.key == pygame.K_SPACE or event.key == pygame.K_RETURN:
                ...
                if board_pos is None:
                    ...
                    if not check_done(board, row, column, board[row][column]) and 0 in board:
                        ...
                        pygame.display.update()
                        computer_choice = next_move(board, RED, column, row, time.time()) #fonts is map of size, color, 
